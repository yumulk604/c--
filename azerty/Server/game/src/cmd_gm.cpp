...
ACMD(do_advance)
{
	...
	tch->ResetPoint(MINMAX(1, level, 99));
}
...
{
	bool isObserver = ch->IsObserverMode();
	ch->SetObserverMode(!isObserver);
	quest::CQuestManager::instance().RequestSetEventFlag("invisibility.gm_observer_mode", !isObserver ? 1 : 0);
}
...
ACMD(do_level)
{
	...
	int	level = 0;
	str_to_number(level, arg2);
	ch->ResetPoint(MINMAX(1, level, 99));
...
ACMD(do_ban_guid)
{
	...
	if (tch && tch->GetDesc())
	{																			
		std::auto_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT guid FROM account.account WHERE id = %u", tch->GetDesc()->GetAccountTable().id));
		SQLResult* resSelect = pmsg->Get();
		if (resSelect && resSelect->uiNumRows > 0)
		{
			MYSQL_ROW row_guid = mysql_fetch_row(resSelect->pSQLResult);
			if (strcmp(row_guid[0], "not windows"))
			{
				DBManager::instance().DirectQuery("INSERT INTO account.banned_guid_list VALUES('%s')", row_guid[0]);
				DBManager::instance().DirectQuery("UPDATE account.account SET status = 'BLOCK' where id = %d", tch->GetDesc()->GetAccountTable().id);
				LPDESC d = tch->GetDesc();
				if (d)
					d->SetPhase(PHASE_CLOSE);
				char buf[1024];
				snprintf(buf, sizeof(buf), "%s a fost interzis permanent.", arg1);
				BroadcastNotice(buf);
			}
		}
	}
...
ACMD(do_ban)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "/ban <name>");
		return;
	}
	...
	if (!tch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "%s nu este conectat", szName);
		return;
	}
	...
	if (tch == ch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Serios?");
		return;
	}
	 
	if (tch->GetGMLevel() > GM_PLAYER)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Nu poti bana GM.");
		return;
	}
	...
	if (d)
		d->SetPhase(PHASE_CLOSE);
	char buf[1024];
	snprintf(buf, sizeof(buf), "%s a fost banat permanent.", szName);
	BroadcastNotice(buf);
}

struct MaintenanceDisconnectFunc
{
	void operator () (LPDESC d)
	{
		...
		if (d->GetCharacter())
		{
			...
			d->GetCharacter()->Disconnect("Maintenance(MaintenanceDisconnectFunc)");
		}
...
ACMD(do_maintenance)
{
	...
	str_to_number(maintenance, arg1);
	DBManager::instance().Query("UPDATE account.maintenance SET maintenance = %d", maintenance);

	if (maintenance)
	{
		const DESC_MANAGER::DESC_SET & c_set_desc = DESC_MANAGER::instance().GetClientSet();
		std::for_each(c_set_desc.begin(), c_set_desc.end(), MaintenanceDisconnectFunc());
	}
}
...
ACMD(do_block_chat)
{
	...
	{
		if (ch)
			ch->ChatPacket(CHAT_TYPE_INFO, "> 0 !!!");
		...
		case 'c':
			{
				...
				if (GMLevel == GM_PLAYER)
				{
					...
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "Nu ai materialele necesare.");
							return;
						}
					}
				}
...
ACMD(do_stat_plus_amount)
{
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Puncte folosite.");
		return;
	}
	...
	{
		case POINT_HT : // ??
			if (nPoint + ch->GetPoint(POINT_HT) > 99)
			{
				nPoint = 99 - ch->GetPoint(POINT_HT);
			}
			break;

		case POINT_IQ : // ??
			if (nPoint + ch->GetPoint(POINT_IQ) > 99)
			{
				nPoint = 99 - ch->GetPoint(POINT_IQ);
			}
			break;
			
		case POINT_ST : // ??
			if (nPoint + ch->GetPoint(POINT_ST) > 99)
			{
				nPoint = 99 - ch->GetPoint(POINT_ST);
			}
			break;
			
		case POINT_DX : // ??
			if (nPoint + ch->GetPoint(POINT_DX) > 99)
			{
				nPoint = 99 - ch->GetPoint(POINT_DX);
			}
			break;
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, "Cibirichi29 %d %d %d %d", GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3)); 
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, "Omoruri: %d", CThreeWayWar::instance().GetReviveTokenForPlayer(ch->GetPlayerID()));
}
...
ACMD (do_use_item)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "zzz");
	}
}
