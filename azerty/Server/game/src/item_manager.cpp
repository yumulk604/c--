...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, DWORD count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	...
	else
	{
		...
		if (item->GetType() == ITEM_UNIQUE)
		{
			...
			else
			{
				...
				item->SetSocket(ITEM_SOCKET_UNIQUE_REMAIN_TIME, get_global_time() + item->GetValue(2));
			}
		}
	}
	...
	if (((item->GetWearFlag() & WEARABLE_UNIQUE && item->IsEquipped()) || !(item->GetWearFlag() & WEARABLE_UNIQUE)) && item->GetValue(0))
	...
	if (id == 0)
	{
		...
		if (vnum == 50300 || vnum == ITEM_SKILLFORGET_VNUM)
		{
			...
			} while (!CSkillManager::instance().Get(dwSkillVnum));
...
bool ITEM_MANAGER::CreateDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item)
{
	...
	if (GetDropPerKillPct(100, 1000, iDeltaPercent, "lotto_drop") >= number(1, iRandRange))
	{
		...
		DBManager::instance().ReturnQuery(QID_LOTTO, pkKiller->GetPlayerID(), pdw,
				"INSERT INTO lotto_list VALUES(0, 'server', %u, NOW())",
				pkKiller->GetPlayerID());
	}
...
bool ITEM_MANAGER::CreateDropItemVector(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<std::pair<int,int> > & vec_item)
{
	...
	itertype(m_map_pkDropItemGroup) itt = m_map_pkDropItemGroup.find(pkChr->GetRaceNum());
	if (itt != m_map_pkDropItemGroup.end())
	{
		typeof(itt->second->GetVector()) v = itt->second->GetVector();
		for (DWORD i = 0; i < v.size(); ++i)
		{
			if (v[i].dwVnum >= 70104 && v[i].dwVnum <= 70107)
				if (v[i].dwVnum != pkChr->GetPolymorphItemVnum())
					continue;
	...
	itertype(m_map_pkMobItemGroup) ittt = m_map_pkMobItemGroup.find(pkChr->GetRaceNum());
	if (ittt != m_map_pkMobItemGroup.end())
	{
		CMobItemGroup* pGroup = ittt->second;
		if (pGroup && !pGroup->IsEmpty())
		{
			const CMobItemGroup::SMobItemGroupInfo& info = pGroup->GetOne();
	...
	itertype(m_map_pkLevelItemGroup) itttt = m_map_pkLevelItemGroup.find(pkChr->GetRaceNum());
	if (itttt != m_map_pkLevelItemGroup.end())
		if (itttt->second->GetLevelLimit() <= (DWORD)iLevel)
		{
			typeof(itttt->second->GetVector()) v = itttt->second->GetVector();
			for (DWORD i = 0; i < v.size(); ++i)
				vec_item.push_back(std::make_pair(v[i].dwVNum, v[i].iCount));
		}
	...
	{
		itertype(m_map_pkGloveItemGroup) ittttt = m_map_pkGloveItemGroup.find(pkChr->GetRaceNum());
		if (ittttt != m_map_pkGloveItemGroup.end())
		{
			typeof(ittttt->second->GetVector()) v = ittttt->second->GetVector();
			for (DWORD i = 0; i < v.size(); ++i)
				vec_item.push_back(std::make_pair(v[i].dwVnum, v[i].iCount));
		}
	}
	...
	if (pkChr->GetMobDropItemVnum())
	{
		itertype(m_map_dwEtcItemDropProb) itttttt = m_map_dwEtcItemDropProb.find(pkChr->GetMobDropItemVnum());
		if (itttttt != m_map_dwEtcItemDropProb.end())
...
static void __DropEvent_CharStone_DropItem(CHARACTER & killer, CHARACTER & victim, ITEM_MANAGER& itemMgr, std::vector<LPITEM>& vec_item)
{
	...
	{
		int log_level = (killer.GetGMLevel() >= GM_LOW_WIZARD) ? 0 : 1;
