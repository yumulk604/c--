...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	if (GetLevel() < 10)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, "Nu ai pierdut experienþã.");
		return;
	}
	...
	if (IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_DEATH_PENALTY))
	{
		...
		int iLoss = ((GetNextExp() * aiExpLossPercents[MINMAX(1, GetLevel(), 99)]) / 100);
...
void CHARACTER::Reward(bool bItemDrop)
{
	...
	if (pkAttacker->IsPC())
	{
		if ((GetLevel() + 10) >= pkAttacker->GetLevel())
		{
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && !isAgreedPVP && !isUnderGuildWar && IsPC() && !isDuel && !isForked && !IS_CASTLE_MAP(GetMapIndex()))
		if (!IsGM())
			ItemDropPenalty(pkKiller);

	// CASTLE_SIEGE
	if (pkKiller && IS_CASTLE_MAP(GetMapIndex()))
	{
			...
			else
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked)
				{
					...
					if (number(1, 100) < iNoPenaltyProb)
						pkKiller->ChatPacket(CHAT_TYPE_INFO, "Nu ai pierdut obiecte.");
					else
					{
		...
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			...
			else
			{
				if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
				{
					...
					pkKiller->ChatPacket(CHAT_TYPE_INFO, "În timpul rãzboiului experienþa nu se aplicã.");
				}
			}
		}
	}
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	switch (to->GetMountVnum())
	{
		...
		case 20112:
		case 20113:
			if (to->IsEquipUniqueItem(71115) || to->IsEquipUniqueItem(71117) || to->IsEquipUniqueItem(71119) || to->IsEquipUniqueItem(71121))
				iExp += iExp * 10 / 100;
			break;
	...
	if (you)
	{
		...
		if (to->GetPremiumRemainSeconds(PREMIUM_MARRIAGE_FAST) > 0 || you->GetPremiumRemainSeconds(PREMIUM_MARRIAGE_FAST) > 0)
...
int CHARACTER::GetArrowAndBow(LPITEM * ppkBow, LPITEM * ppkArrow, int iArrowCount/* = 1 */)
{
	...
	LPITEM pkArrow = GetWear(WEAR_ARROW);
	if (!pkArrow || pkArrow->GetType() != ITEM_WEAPON || pkArrow->GetProto()->bSubType != WEAPON_ARROW)
		return 0;
				...
				case 1: // ÀÏ¹Ý ¸¶¹ý
					{
						...
						if (m_me->GetJob() == JOB_SURA || m_me->GetJob() == JOB_SHAMAN) 
							iDam = iDam * (85 - pkVictim->GetPoint(POINT_RESIST_MAGIC)) / 100;
						else
						...
						pkVictim->Damage(m_me, abs(iDam), DAMAGE_TYPE_MAGIC);
						// Å¸°ÝÄ¡ °è»êºÎ ³¡
					}
					break;
