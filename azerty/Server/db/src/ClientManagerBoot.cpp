...
bool CClientManager::InitializeRefineTable()
{
	...
	snprintf(query, sizeof(query), "SELECT id, cost, prob, vnum0, count0, vnum1, count1, vnum2, count2,  vnum3, count3, vnum4, count4 FROM refine_proto");
	...
	while (data.Next())
	{
		...
		else 
		{
			if (!Set_Proto_Mob_Table(mob_table, data, localMap))
				fprintf(stderr, "mob table error\n");		
		}
	...
	while (data.Next())
	{
		...
		if(it_map_itemTable == test_map_itemTableByVnum.end())
		{
			if (!Set_Proto_Item_Table(item_table, data, localMap))
				fprintf(stderr, "item table error\n");
		} 
		else 
		{
...
{
	char query[4096];
	snprintf(query, sizeof(query), "SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear FROM item_attr ORDER BY apply");
...
{
	char query[4096];
	snprintf(query, sizeof(query), "SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear FROM item_attr_rare ORDER BY apply");
...
bool CClientManager::InitializeLandTable()
{
	...
	snprintf(query, sizeof(query), "SELECT id, map_index, x, y, width, height, guild_id, guild_level_limit, price FROM land WHERE enable='YES' ORDER BY id");
