...
bool CClientManager::Initialize()
{
	...
	CDBManager::instance().DirectQuery("CREATE TABLE IF NOT EXISTS `kingdoms` (`id` int(11) unsigned NOT NULL AUTO_INCREMENT, `name` varchar(255) NOT NULL, `flag` varchar(255) NOT NULL, `map_data` text NOT NULL, `player_id` int(11) unsigned NOT NULL, PRIMARY KEY (`id`), KEY `player_id` (`player_id`), CONSTRAINT `kingdoms_ibfk_1` FOREIGN KEY (`player_id`) REFERENCES `player` (`id`) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
	CDBManager::instance().DirectQuery("ALTER TABLE `player` ADD COLUMN `kingdom_id` INT(11) UNSIGNED DEFAULT NULL;");
	CDBManager::instance().DirectQuery("ALTER TABLE `player` ADD CONSTRAINT `player_ibfk_1` FOREIGN KEY (`kingdom_id`) REFERENCES `kingdoms` (`id`) ON DELETE SET NULL;");

	CGuildManager::instance().BootReserveWar();

	m_fdAccept = socket_tcp_bind("127.0.0.1", 11000);
	if (m_fdAccept < 0)
	{
		perror("socket");
		return false;
	...
	m_iPlayerDeleteLevelLimit = 99 + 1;
	m_iPlayerDeleteLevelLimitLower = 0;
...
void CClientManager::QUERY_QUEST_SAVE(CPeer * pkPeer, TQuestTable * pTable, DWORD dwLen)
{
	...
	for (int i = 0; i < iSize; ++i, ++pTable)
	{
		...
		{
			snprintf(szQuery, sizeof(szQuery),
					"REPLACE INTO quest (dwPID, szName, szState, lValue) VALUES(%d, '%s', '%s', %ld)",
...
void CClientManager::QUERY_SAFEBOX_CHANGE_SIZE(CPeer * pkPeer, DWORD dwHandle, TSafeboxChangeSizePacket * p)
{
	...
	if (p->bSize == 1)
		snprintf(szQuery, sizeof(szQuery), "INSERT INTO safebox (account_id, size) VALUES(%u, %u)", p->dwID, p->bSize);
	else
...
void CClientManager::QUERY_EMPIRE_SELECT(CPeer * pkPeer, DWORD dwHandle, TEmpireSelectPacket * p)
{
	...
	{
		snprintf(szQuery, sizeof(szQuery),
				"SELECT pid1, pid2, pid3, pid4 FROM player_index WHERE id=%u", p->dwAccountID);
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == SAFEBOX || p->window == MALL)
	{
		...
		snprintf(szQuery, sizeof(szQuery), 
			"REPLACE INTO item (id, owner_id, window, pos, count, vnum, socket0, socket1, socket2, "
			"attrtype0, attrvalue0, "
			"attrtype1, attrvalue1, "
			"attrtype2, attrvalue2, "
			"attrtype3, attrvalue3, "
...
void CClientManager::MyshopPricelistUpdate(const TPacketMyshopPricelistHeader* pPacket)
{
	...
	else
	{
		...
		char szQuery[QUERY_MAX_LEN];
		snprintf(szQuery, sizeof(szQuery), "SELECT item_vnum, price FROM myshop_pricelist WHERE owner_id=%u", pPacket->dwOwnerID);
...
{
	DWORD dwMin = 2000000000, dwMax = 2100000000;
	...
	for (int n = 0; (row = mysql_fetch_row(pMsg->Get()->pSQLResult)) != NULL; ++n)
	{
		...
		if (strcmp(locale.szKey, "LOCALE") == 0)
		{
			...
			if (g_stLocale != locale.szValue)
				sys_log(0, "Changed g_stLocale %s to latin2", g_stLocale.c_str());
